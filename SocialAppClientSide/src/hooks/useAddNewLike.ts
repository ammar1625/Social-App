...
function addNewLike(model:likeToAddModel)
{
    return axiosInstance.post<postModel[]>(`/Like/add-new`,model).then(res=>res.data);
}

export function useAddNewLike(queryClient:QueryClient , userId:string ,targetUserId:string, type:number  )
{
    return useMutation({
